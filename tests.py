...
def test_create_wallet(test_app, truncate_db):
    response = test_app.post("/wallet", json={"client_id": 1, "currency": "USD"})
    assert response.status_code == 200
...
def test_add_money(test_app, truncate_db):
    response = test_app.post("/wallet", json={"client_id": 101, "currency": "USD"})
    assert response.status_code == 200
...
def test_send_money(test_app, truncate_db):
    ...
    response = test_app.post("/wallet", json={"client_id": 2, "currency": "USD"})
    assert response.status_code == 200
    ...
    assert response.json() == {
        ...
        "result": {
            ...
            "wallet_to": {"id": 2, "client_id": 2, "amount": 500.0, "currency": "USD"},
            "tr": {
                "id": 2,
                "from_wallet_id": 1,
                "to_wallet_id": 2,
...
def test_send_money_self_sending(test_app, truncate_db):
    ...
    assert response.json() == {
        "success": False,
        "errors": [
            {"loc": ["body", "__root__"], "msg": "self sending", "type": "value_error"}
        ],
        "result": None,
    }
...
def test_send_money_not_enough_money_wallet_id_from_not_found(test_app, truncate_db):
    request_id = str(uuid.uuid4())
...
def test_send_money_not_enough_money_wallet_id_to_not_found(test_app, truncate_db):
    response = test_app.post("/wallet", json={"client_id": 1, "currency": "USD"})
    assert response.status_code == 200
    wallet_id_from = response.json()["result"]["wallet"]["id"]
